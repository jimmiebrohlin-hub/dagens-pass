-- Fix shared streak RLS recursion by moving membership checks into a SECURITY DEFINER helper.
-- The original shared_streak_members select policy queried shared_streak_members from within
-- its own policy, which can trigger recursive RLS evaluation.
...
-- Replace policies that depend on membership with non-recursive helper checks.
