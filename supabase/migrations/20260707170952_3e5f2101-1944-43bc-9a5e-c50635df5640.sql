...
begin
  ...
  insert into public.shared_streaks (id, name, streak_kind, current_turn_user_id, streak_count, created_by)
  values (v_streak_id, 'Min streak', 'personal', v_user_id, 0, v_user_id);

  insert into public.shared_streak_members (streak_id, user_id, display_name, member_order, role, status)
  values (v_streak_id, v_user_id, v_display_name, 0, 'owner', 'active');

  insert into public.shared_streak_activity (streak_id, user_id, activity_type, to_user_id, streak_count_after)
  values (v_streak_id, v_user_id, 'created', v_user_id, 0);
...
begin
  ...
  insert into public.shared_streaks (id, name, streak_kind, current_turn_user_id, streak_count, created_by)
  values (v_streak_id, coalesce(nullif(trim(p_name), ''), 'Streak med någon'), 'buddy', v_user_id, 0, v_user_id);
...
begin
  ...
  select id, streak_kind into v_streak_id, v_streak_kind
  from public.shared_streaks
  where invite_code = upper(trim(p_invite_code)) and status = 'active'
  limit 1;

  if v_streak_id is null then raise exception 'invite_not_found'; end if;

  if exists (select 1 from public.shared_streak_members
             where streak_id = v_streak_id and user_id = v_user_id and status = 'active') then
    return v_streak_id;
  end if;

  if v_streak_kind = 'personal' then raise exception 'personal_streak_cannot_be_joined'; end if;

  select count(*) into v_member_count from public.shared_streak_members
  where streak_id = v_streak_id and status = 'active';
  if v_member_count >= 2 then raise exception 'streak_full'; end if;

  select coalesce(raw_user_meta_data->>'full_name', raw_user_meta_data->>'name', email)
    into v_display_name from auth.users where id = v_user_id;

  select coalesce(max(member_order), -1) + 1 into v_next_order
  from public.shared_streak_members where streak_id = v_streak_id;

  insert into public.shared_streak_members (streak_id, user_id, display_name, member_order, role, status)
  values (v_streak_id, v_user_id, v_display_name, v_next_order, 'member', 'active');

  insert into public.shared_streak_activity (streak_id, user_id, activity_type, to_user_id)
  values (v_streak_id, v_user_id, 'joined', v_user_id);
...
begin
  ...
  from (
    select
      ...
      jsonb_build_object(
        'id', s.id, 'name', s.name, 'streak_kind', s.streak_kind, 'status', s.status,
        'current_turn_user_id', s.current_turn_user_id, 'streak_count', s.streak_count,
        'invite_code', s.invite_code, 'created_by', s.created_by,
        'last_completed_at', s.last_completed_at,
        'members', coalesce((
          select jsonb_agg(jsonb_build_object(
            'user_id', sm.user_id, 'display_name', sm.display_name,
            'member_order', sm.member_order, 'role', sm.role, 'status', sm.status
          ) order by sm.member_order asc)
          from public.shared_streak_members sm where sm.streak_id = s.id
        ), '[]'::jsonb)
      ) as streak_json
    from public.shared_streak_members m
    join public.shared_streaks s on s.id = m.streak_id
    where m.user_id = v_user_id and m.status = 'active' and s.status = 'active'
  ) q;
...
begin
  ...
  for v_streak in
    select s.* from public.shared_streaks s
    join public.shared_streak_members m on m.streak_id = s.id
    where m.user_id = v_user_id and m.status = 'active' and s.status = 'active'
  ...
  loop
    ...
    where sm.streak_id = v_streak.id and sm.status = 'active';

    if v_streak.streak_kind <> 'personal' and v_active_member_count < 2 then
      v_skipped_not_turn := v_skipped_not_turn + 1; continue;
    end if;
    ...
    if v_streak.streak_kind = 'personal' then
      ...
      select sm.user_id into v_next_user_id
      from public.shared_streak_members sm
      where sm.streak_id = v_streak.id and sm.status = 'active'
        and sm.member_order > coalesce((
          select current_member.member_order from public.shared_streak_members current_member
          where current_member.streak_id = v_streak.id and current_member.user_id = v_user_id limit 1
        ), -1)
      order by sm.member_order asc limit 1;
      ...
      if v_streak.last_completed_at is not null and now() > v_streak.last_completed_at + interval '36 hours' then
        v_new_count := 1; v_reset_count := v_reset_count + 1;
      else
        v_new_count := v_streak.streak_count + 1;
      end if;
    end if;
    ...
    update public.shared_streaks
       set streak_count = v_new_count, last_completed_at = now(),
           current_turn_user_id = v_next_user_id, updated_at = now()
     where id = v_streak.id;

    insert into public.shared_streak_activity (streak_id, user_id, activity_type, from_user_id, to_user_id, streak_count_after)
    values (v_streak.id, v_user_id, 'daily3_done', v_user_id, v_next_user_id, v_new_count);

    v_updated := v_updated || jsonb_build_array(jsonb_build_object(
      'streak_id', v_streak.id, 'streak_count_after', v_new_count,
      'to_user_id', v_next_user_id, 'streak_kind', v_streak.streak_kind,
      'reset_by_timeout', v_streak.streak_kind <> 'personal' and v_streak.last_completed_at is not null and now() > v_streak.last_completed_at + interval '36 hours'
    ));
  end loop;

  return jsonb_build_object(
    'updated', v_updated, 'updated_count', jsonb_array_length(v_updated),
    'skipped_today', v_skipped_today, 'skipped_not_turn', v_skipped_not_turn,
    'reset_count', v_reset_count, 'buddy_window_hours', 36
  );
end;
$$;
grant execute on function public.complete_shared_daily3_turns() to authenticated;

-- Full wipe of all streak tables
...
-- Debug reset RPC for signed-in users
