-- Allow the next member to send the ball back on the same day.
-- A user may advance each streak at most once per day, but another member may advance it later the same day.
...
begin
  ...
  loop
    ...
    if exists (
      select 1
      from public.shared_streak_activity a
      where a.streak_id = v_streak.id
        and a.user_id = v_user_id
        and a.activity_type = 'daily3_done'
